...
orders.push({
  orderId: line.slice(...ORDER_LAYOUT.orderId).trim(),
  customerId: line.slice(...ORDER_LAYOUT.customerId).trim(),
  customerName: line.slice(...ORDER_LAYOUT.customerName).trim(),
  orderDate: toIsoDate(line.slice(...ORDER_LAYOUT.orderDate).trim()),
  amount: toDecimalString(line.slice(...ORDER_LAYOUT.amount).trim()),
  currency: line.slice(...ORDER_LAYOUT.currency).trim(),
});
}
...
const unmatchedCustomers = [...customers.keys()].filter(
  (customerId) => !orderedCustomerIds.has(customerId),
);
